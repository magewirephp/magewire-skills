...
`SupportMagewireRateLimiting` is disabled by default. Its request variant runs before component reconstruction and exposes shared/isolated scope, maximum attempts, and decay. Its component variant is fixed at 4 attempts per 5 seconds in Magewire 3.5. There is no component rate-limit attribute.

## Magewire 3.4–3.5 additions

- `#[Magewirephp\Magewire\Attributes\Lazy]` or the `magewire:component:lazy` layout argument enables `on-intersect`/`on-load` lazy components. Layout values override the attribute; `false` disables it. Lazy components are isolated unless `isolate: false`.
- `Magewirephp\Magewire\app()` resolves through the Magento-backed `ApplicationContainer`. Prefer Magento constructor injection in application code; use `app()` for ported/runtime compatibility code.
- `RequestFilterInterface::check(RequestContext $context)` runs once per update request before reconstruction. Reject only with a `RequestFilterException` subclass so the generic safe response bridge can handle status and severity.
...
Register a custom **addon** (reactive shared state, accessible as `window.MagewireAddons.myAddon`):

```javascript
window.MagewireAddons.register('myAddon', function() {

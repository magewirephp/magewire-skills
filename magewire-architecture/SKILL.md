...
description: "Internals and extension guide for Magewire 3: directory layout, Mechanisms vs Features, area-scoped DI, snapshots, request filters, the application container, layout nodes, and JavaScript extension points. Use when extending or debugging the framework itself."
license: MIT
metadata:
...
Theme and admin integrations moved to standalone packages in 3.2. Use `magewirephp/magewire-hyva-theme`, `magewirephp/magewire-hyva-checkout`, and `magewirephp/magewire-admin`; there is no current in-tree `themes/` directory. A class in `dist/` is not necessarily registered—runtime DI and `Component` concerns determine support.
...
| 1500 | `HandleCompiling` | Compiles templates and directives into `var/magewire/views/{area}/` |
...
| 1250 | `SupportLazyLoading` | Lazy placeholders and activation |
...
class SupportMyFeature extends \Magewirephp\Magewire\ComponentHook
{
    ...
    {
        on('magewire:component:construct', function ($block) {
...
## Service Resolution and Request Filters

Magewire removed experimental Feature/Mechanism facades in 3.3. Do not register a `facade` key.

Prefer Magento constructor injection. Magewire 3.5 also exposes `Magewirephp\Magewire\app()`, backed by `ApplicationContainer`, for ported/runtime compatibility code. It resolves Magento preferences, virtual types, Magewire aliases, and runtime `singleton()`/`instance()` bindings.

Request-wide checks implement `Magewirephp\Magewire\Mechanisms\HandleRequests\Filter\RequestFilterInterface`. The pipeline calls `check(RequestContext $context)` once, in DI order, before any component is reconstructed. Reject with a `RequestFilterException` subclass; keep filters cheap and never hydrate or render in them.
...
Magewire's maintained integrations are standalone Composer packages: `magewirephp/magewire-hyva-theme`, `magewirephp/magewire-hyva-checkout`, and `magewirephp/magewire-admin`. Core is theme-agnostic; Luma and Breeze require a project/community integration.

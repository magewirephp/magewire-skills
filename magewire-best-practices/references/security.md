...
`SupportMagewireRateLimiting` is disabled by default. The request variant runs once before reconstruction and has configurable shared/isolated scope, maximum attempts, and decay. The component variant is fixed at 4 attempts per 5 seconds in Magewire 3.5. There is no component rate-limit attribute. Rate limiting limits volume; authorization and validation remain mandatory.

For other inexpensive request-wide rejection rules, implement `RequestFilterInterface` and register it in the 3.5 request-filter pipeline.
